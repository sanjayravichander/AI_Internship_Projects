...
Version: Final with reliable translation
"""
...
try:
    ...
    # Create a dummy GoogleTranslator class
...
def main():
    ...
    if not TRANSLATION_AVAILABLE:
        st.warning("⚠️ Translation features are disabled. Install deep-translator for full functionality: `pip install deep-translator`")
...
def display_language_settings_disabled():
    ...
    st.selectbox(
        ...
        list(LANGUAGE_MAP.keys()),
        disabled=True,
        help="Install deep-translator to enable translation"
    )
                ...
                if 'error' not in results:
                    ...
                    for i, result in enumerate(results['results']):
                        with st.expander(f"📄 Result #{i+1} - {result.get('document_name', 'Unknown')} (Score: {result['relevance_score']:.3f})"):
                            ...
                            if TRANSLATION_AVAILABLE and st.session_state.target_language != 'en':
                                if st.button(f"🌐 Translate", key=f"translate_result_{i}"):
                ...
                if 'error' not in results:
                    ...
                    if TRANSLATION_AVAILABLE and st.session_state.target_language != 'en':
                        if st.button("🌐 Translate Results"):
                            text_to_translate = results.get('analysis') or results.get('comparison', '')
                            if text_to_translate:
                                translated = translate_text(text_to_translate, st.session_state.target_language)
                                if translated:
